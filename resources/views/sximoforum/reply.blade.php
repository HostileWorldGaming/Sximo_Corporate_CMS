...
<div class="sbox animated fadeInRight">
  <div class="sbox-title"> <h4> <i class="fa fa-comment"></i>  All Forum : <small>{{ $row->forumTitle }}  : {{ $row->Topic }} : {{ $row->Title }} </small></h4>
    <div class="sbox-tools" >
       <a href="{{ url('sximoforum/topic/'.$row->CategoryID)}}" class="btn btn-white tips" title="Back To Posts"><i class="fa fa-arrow-circle-left"></i> </a>
    </div>
  </div>
            ...
            <td>
              <div class="displayPost">
               ...
               <?php echo  SiteHelpers::BBCode2Html( strip_tags($row->Content)) ; ?>
                <hr />
                Posted : {{ date('M j, Y',strtotime($row->Posted)) }} <br />
              </div>
              <div class="displayEdit" style="display:none; ">
                ...
                <a href="{{ url::to('sximoforum/deletepost/'.$row->PostID.'/'.$row->CategoryID)}}" class="text-danger  confirmDelete"><i class="fa fa-trash-o"></i> Delete Post </a> <br /><br /><br />
                @endif 
          ...
          <tr>
            ...
            <td>
               ...
               @if(\Session::get('uid') == $comm->UserID or \Session::get('gid') ==1) 
                ...
                <a href="{{ url::to('sximoforum/deletereply/'.$comm->CommentID.'/'.$row->PostID)}}" class="text-danger confirmDelete"><i class="fa fa-trash-o"></i> Delete Comment </a> <br /><br /><br />
                @endif            
            </td>
          </tr>
